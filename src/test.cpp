...
const unsigned EVT_BUTTON_ACTIVATE = 810;
const unsigned EVT_CALC_BUTTON_ACTIVATE = 811;
const unsigned EVT_TEXTBOX_POSITION = 812;
const unsigned EVT_MENU_SELECT = 813;
const unsigned EVT_LIST_ACTION = 814;
const unsigned EVT_KEYBOARD = 815;
...
protected:
	...
	virtual void onTouchLong(unsigned x, unsigned y) {}
...
public:
	...
	void update() {
		...
		//TODO: assuming that global font is used for all widgets
...
private:
	...
	void onTouchLong(unsigned x, unsigned y) {
		if(getFocus() != this) {
			setFocus(this, true);
			InvertAreaBW(m_x, m_y, m_w, m_h);
			update();
...
public:
	...
	void show() {
		...
		OpenList(m_title.c_str(), NULL,
				 ...
				 ((m_items->size() == 0) ? 1 : m_items->size()), //OpenList doesn't work when number of items is 0
				 0, &listCallback
		);
	}
...
private:
	static int listCallback(int action, int x, int y, int idx, int state) {
		...
		if(action == LIST_PAINT && m_visible_list->m_items->size() != 0) {
			...
			DrawTextRect(x+2*c_wpad, y, ScreenWidth() - 4*c_wpad, menu_font->height + c_hpad/2,
		...
		if(m_visible_list->m_items->size() == 0 && action == LIST_MENU) {
			...
			SendEvent(&global_event_handler, EVT_LIST_ACTION, (m_visible_list->m_id << 16) | 0xffff, LIST_MENU);
			return 1;
		}
	...
	static FullscreenList* m_visible_list; //TODO: this looks like a hack (or doesnt?)
...
public:
	Application() {
		...
		m_variables[0] = 0.0; m_variables[1] = 0.0;
		m_variables[2] = 0.0; m_variables[3] = 0.0;
		m_variables[4] = 0.0;
		...
		m_menu->append(ITEM_ACTIVE, c_menu_eval, "Evaluate");
		...
		for(uint i = 0; i < m_history.size(); i++)
			m_historyList->append(m_history[i].c_str());
		
		m_buttonsLayout = new GridLayout(
			c_padding, m_answerBox->getY() + m_answerBox->getHeight() + c_padding,
			CFG_GRID_ROWS, CFG_GRID_COLS,
			ScreenWidth() - c_padding*2, ScreenHeight() - c_padding*2 - m_answerBox->getY() - m_answerBox->getHeight() - c_padding,
			c_layout_sp);
		
		for(unsigned row = 0; row < CFG_GRID_ROWS; row++) {
			for(unsigned col = 0; col < CFG_GRID_COLS; col++) {
				if(functions[row][col].name[0] != '\0') {
					Button* tmp = new CalcButton(&functions[row][col]);
	...
	int event(int event, int param1, int param2) {
		...
		case EVT_CALC_BUTTON_ACTIVATE:
		{
			...
			/* TODO: insert normal "cnst" button (not CalcButton) to GridLayout */
			if(btn->getLabel() == "cnst") {
				m_menu->show();
				break;
			}
		...
		case EVT_BUTTON_ACTIVATE:
		{
		    if((unsigned)param1 == m_buttonClear->getID() && param2) {
				...
				int parser_result = m_fparser->Parse(m_inputBox->getString(),"ans,a,b,c,d");
				m_answerBox->words().clear();
				double result = m_fparser->Eval(m_variables);
				
				if(parser_result != -1) {
					m_answerBox->words().push_back(m_fparser->ErrorMsg());
					m_answerBox->draw();
				...
				if(m_fparser->EvalError() == 0) {
					...
					m_variables[0] = result;
					
					m_inputBox->clear();
					m_inputBox->update();
				}
				else {
					m_answerBox->words().push_back("Evaluation error");
				}
		...
		case EVT_MENU_SELECT:
			if(param1 == 0) { //app menu
				switch(param2) {
					...
					break;
					case c_menu_eval: {
						Keyboard::show("Enter expression to evaluate","2+2",c_menu_eval);
					}
					break;
					case c_menu_custom: {
						m_exprList->show();
		...
		case EVT_KEYBOARD:
			...
			else if(param2 == c_menu_list_edit) {
				try {
					string name, body, var;
			...
			else if(param2 == c_menu_eval) {
				//TODO: create parseExpression func or smth
				int parser_result = m_fparser->Parse(Keyboard::getText(), "ans,a,b,c,d");
				m_answerBox->words().clear();
				double result = m_fparser->Eval(m_variables);
...
private:
	void initParser() {
		...
		for(uint i = 0; i < m_customExpr.size(); i++) {
			...
			addExpression(name, m_customExpr[i].second, variables);
		}
	}
	...
	void readConfig() {
		...
		if(cfg == NULL)
			std::cout << "NULL config" << std::endl;
		
		char* expr, *history, *hist_ent;
		
		char* expressions = ReadString(cfg, "expressions", NULL);
		if(expressions == NULL)
			goto create_config;
		
		vector<string> expr_vec = splitStr(expressions,"\n");
		for(uint i = 0; i < expr_vec.size(); i++) {
			string name, body, variables;
			parseExpression(expr, name, body, variables);
		...
		vector<string> hist_vec = splitStr(history,"\n");
		hist_ent = strtok(history, "\n");
		while(hist_ent != NULL && *hist_ent != '\0') {
	...
	void writeConfig() {
		...
		expressions[strlen(expressions)]='\0';
		...
		uint hist_size = 0;
		for(uint i = 0; i < m_history.size(); i++)
			hist_size += m_history[i].size() + 1;
		hist_size++;
		
		char* history = new char[hist_size]; history[0] = '\0';
		for(uint i = 0; i < m_history.size(); i++) {
			strcat(history, m_history[i].c_str());
			strcat(history, "\n");
		}
		history[strlen(expressions)] = '\0';
		delete [] history;
	...
	string addExpression(const string& name, const string& body, const string& variables) {
		...
		m_fparser->AddFunction(name, parser);
	...
	vector<string> splitStr(char* str, const char* delim) {
	...
	vector<vector<string> > m_history;
};

const char Application::c_config[] = "ecalc.cfg";
