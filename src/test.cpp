	...
	int event(int event, int param1, int param2) {
		switch(event) {
			...
			case EVT_LIST_ACTION: {
				if(param2 == LIST_MENU) {
					if((param1 & 0x0000ffff) == 0xffff) {      //no items are selected
					...
					if((uint)(param1 >> 16) == m_exprList->getID()) {
						uint index = param1 & 0x0000ffff;
						string name1;
					...
					else if((uint)(param1 >> 16) == m_historyList->getID()) {
						uint index = param1 & 0x0000ffff;
						m_inputBox->clear();
			...
			case EVT_KEYBOARD:
				...
				else if((uint) param2 == c_menu_list_edit) {
					try {
						string name, body, var;
				...
				else if((uint) param2 == c_menu_eval) {
					evalAndDisplay(Keyboard::getText());
					vector<string> hist_ent;
					for(string::iterator it = Keyboard::getText().begin(); it != Keyboard::getText().end(); it++)
